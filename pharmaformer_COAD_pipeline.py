...
if PHASE == '':
    ...
    print('='*60)
    print('PharmaTFormer COAD Pipeline')
    print('='*60)
...
MODEL_PATH = os.path.join(output_dir, 'ensemble_model_COAD.pth')
...
def run_pretrain():
    ...
    expr = pd.read_csv(f'{dataDir}/ccle/ccle_rnaseq_COAD.csv', index_col=0)
...
def run_finetune():
    ...
    def load_organoid(gpath, dpath):
        xg = pd.read_csv(gpath, index_col=0); xg.dropna(inplace=True)
    ...
    def finetune(models, xg, xs, y, epochs):
        ...
        for i, m in enumerate(models):
            ...
            opt = optim.Adam(m.parameters(), lr=0.00001, weight_decay=1e-3)
    ...
    def evaluate(models, ctype, drug, tpath):
        expr = pd.read_csv(tpath, index_col=0); expr.dropna(inplace=True)
        ...
        sdf = pd.read_csv(f'{dataDir}/ccle/drug_smiles.csv'); sdf.set_index('Drug Name', inplace=True); sdf.index = sdf.index.str.strip()
        bpe_f = codecs.open(f'{dataDir}/bpe.codes', encoding='utf-8'); bpe = BPE(bpe_f); bpe_f.close()
        enc = [ord(c) for c in bpe.process_line(sdf.loc[drug, 'SMILES'])]
        if len(enc) > 128: enc = enc[:128]
        ...
        rdf = pd.DataFrame({'sample': expr.index, f'{drug}_prediction': predictions})
        clin = pd.read_csv(f'{dataDir}/tcga_reproduce/TCGA_{ctype}_merged_{drug}_survival_1.csv')
        clin = clin[['sample','OS','OS.time']].dropna()
        merged = rdf.merge(clin, on='sample')
        fpr, tpr, th = roc_curve(merged['OS'], merged[f'{drug}_prediction'])
        merged['risk_group'] = (merged[f'{drug}_prediction'] > th[np.argmax(tpr-fpr)]).astype(int)
        cph = CoxPHFitter()
        ...
        plt.title(f'{ctype} {drug} Overall Survival'); plt.xlabel('Time (days)'); plt.ylabel('Survival Probability')
        plt.savefig(os.path.join(output_dir, f'{ctype}_{drug}_survival_curve.png')); plt.close()
        merged.to_csv(os.path.join(output_dir, f'{ctype}_{drug}_predictions_finetune_ensemble.csv'), index=False)

    info = {
        'COAD': {'gpath': f'{dataDir}/organoid/COAD/COAD_organoid_TPM.csv', 'dpath': f'{dataDir}/organoid/COAD/COAD_drug_response.csv',
                 'tpath': f'{dataDir}/tcga_reproduce/TCGA-COAD-gene-adjust.csv', 'drugs': ['5-Fluorouracil', 'Oxaliplatin']}
    }
    for ctype, v in info.items():
        for drug in v['drugs']:
            np.random.seed(seed); torch.manual_seed(seed)
            if torch.cuda.is_available(): torch.cuda.manual_seed(seed)
            random.seed(seed)
            print(f'\n{"="*60}\n{ctype} {drug}\n{"="*60}')
            xg, xs, y = load_organoid(v['gpath'], v['dpath'])
            ...
            evaluate(models, ctype, drug, v['tpath'])
